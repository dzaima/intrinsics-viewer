...
download-riscv: data/rvv_base-1.json data/rvv_policies.json data/v-spec.html
...
data/rvv_base-1.json:
	mkdir -p data
	curl -L 'https://github.com/dzaima/rvv-intrinsic-doc/releases/download/v3/rvv_base.json' > "$@"
data/rvv_policies.json:
	mkdir -p data
	curl -L 'https://github.com/dzaima/rvv-intrinsic-doc/releases/download/v2/rvv_policies.json' > "$@"
